...
abilbotz.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add" && mem.includes(abilbotz.user.jid)) {
        abilbotz.sendMessage(anu.jid, "Halo!.. saya ABIL BOTZ saya akan membatu mempermudah kehidupan..seperti membuat sticker dan lain-lain. untuk memulai silahkan ketik !menu.", "conversation")
      }
      if (!isWelkom) return
      if (anu.action == "add" && !mem.includes(abilbotz.user.jid)) {
